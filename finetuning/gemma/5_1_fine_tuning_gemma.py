...
BASE_MODEL_NAME = "google/gemma-2-2b"  # Asegúrate de seleccionar el modelo adecuado #Qwen/Qwen2.5-1.5B / meta-llama/Llama-3.2-1B / microsoft/phi-1
...
def load_model_and_tokenizer(base_model_name):
    ...
    try:
        ...
        tokenizer = AutoTokenizer.from_pretrained(base_model_name, use_fast=True, clean_up_tokenization_spaces=True, cache_dir="./cache")
...
def prepare_datasets(dataset: Dataset, tokenizer, test_size: float=0.02, validation_size: float=0.996):
    ...
    tokenized_dataset = dataset.map(lambda x: tokenize_preprocess(x, tokenizer), cache_file_name="./cache/tokenized_dataset.arrow", batched=True)
...
def configure_training():
    ...
    training_args = TrainingArguments(
    	...
    	num_train_epochs=2, #Disminuye
    	learning_rate=2e-6, #Disminuye
    	per_device_train_batch_size=8,
    	gradient_accumulation_steps=8, #Disminuye
    	#per_device_eval_batch_size=1, #Quita
        eval_strategy="steps",
    	eval_steps=300,
    	save_steps=300,
    	logging_dir='./logs',
...
if __name__ == "__main__":
    ...
    init_wandb(peft_training_args, PROJECT_NAME_WANDB, MODEL_NAME, FILE_NAME, current_datetime) #gemma
...
# Intenta obtener el estado existente del Accelerator o crear uno nuevo
try:
    # Primero intentamos obtener el estado existente
    state = AcceleratorState()
    accelerator = Accelerator(state=state)
except:
    # Si no hay estado existente, creamos uno nuevo
    accelerator = Accelerator(mixed_precision="no")

device = accelerator.device
print("Dispositivo seleccionado por Accelerate:", device)

# Mover los modelos al dispositivo explícitamente antes de prepararlos
model_base = model_base.to(device)
peft_model_gemma = peft_model_gemma.to(device)

# Preparar los modelos con Accelerator
model_base, peft_model_gemma = accelerator.prepare(model_base, peft_model_gemma)

# Modo evaluación
model_base.eval()
peft_model_gemma.eval()
...
descriptions = tokenized_datasets['test'][0:1000]['description']
human_baseline_test_case = tokenized_datasets['test'][0:1000]['test_case']

# Función auxiliar para generar texto
def generate_test_case(model, inputs, tokenizer):
    with torch.no_grad():
...
for description in descriptions:
    ...
    # Tokenizar y mover al dispositivo correcto
    inputs = tokenizer(prompt, return_tensors="pt", padding=True, truncation=True)
    inputs = {k: v.to(device) for k, v in inputs.items()}
    
    # Debug: verificar dispositivos
    print(f"\nDispositivos actuales:")
    print(f"input_ids: {inputs['input_ids'].device}")
    print(f"attention_mask: {inputs['attention_mask'].device}")
    print(f"model_base: {next(model_base.parameters()).device}")
    print(f"peft_model: {next(peft_model_gemma.parameters()).device}")
    
    # Generar casos de prueba
    base_model_text_output = generate_test_case(model_base, inputs, tokenizer)
    peft_model_text_output = generate_test_case(peft_model_gemma, inputs, tokenizer)
...
if len(human_baseline_test_case) == len(base_model_test_cases) == len(peft_model_test_cases):
    ...
    df_report = pd.DataFrame(
        zipped_summaries, 
        columns=['HUMAN_BASELINE_TEST_CASES', 'BASE_MODEL_TEST_CASES', f'PEFT_{MODEL_NAME.upper()}_TEST_CASES']
    )
else:
