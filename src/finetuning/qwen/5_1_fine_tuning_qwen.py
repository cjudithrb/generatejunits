...
FILE_PATH = '/home/generatejunits/data/dataset350k.tsv'
...
def init_wandb(training_args, project_name, model_name, file_name, current_datetime):
    ...
    print("wandb: ",wandb_api_key)
    if not wandb_api_key:
        raise ValueError("La clave de API de WANDB no está configurada en el entorno.")
...
for description in tqdm(descriptions, desc="Generando casos de prueba"):
    ...
    # Asegurar que los inputs estén en cuda:0
    ...
    with torch.no_grad():
        outputs = model_base.generate(
            **{k: v.to(device) for k, v in inputs.items()},  # Asegurar que los inputs estén en el dispositivo correcto
            max_new_tokens=200,
        ...
        base_model_text = tokenizer.decode(outputs[0], skip_special_tokens=True)
        base_model_test_cases.append(base_model_text)
    
    # Generar con modelo PEFT
    with torch.no_grad():
        ...
        peft_model_text = tokenizer.decode(outputs[0], skip_special_tokens=True)
        peft_model_test_cases.append(peft_model_text)
...
print("BASE MODEL BERTScore Precision Avg:", base_model_bertscore_precision_avg)
...
print("PEFT MODEL BERTScore Precision Avg:", peft_model_bertscore_precision_avg)
...
print("BASE MODEL SacreBLEU:", base_model_results_sacrebleu)
...
print("BASE MODEL CodeBLEU:", base_model_results_codebleu)
